...
typedef struct MiLo_mod_settings_s
{
	...
	uint8_t PayloadLength;      // Number of OTA bytes to be sent.//8
} MiLo_mod_settings_t;
...
void  MiLo_SetRFLinkRate(uint8_t index) // Set speed of RF link (hz) index values are from 0 to 2 and 2 is binding rate.
{
...
1.txid1
2.txid2
3. no. of bytes in sport frame(on max 4bits) | telemetry uplink counter(4 bits)
4.Sport data byte1
5.Sport data byte 2
6.Sport data byte 3
7.Sport data byte 4
...
0 - downlink telemetry/0
1- RC channels 1_8_1 /1        
2- RC channels 1_8_2/  2      
3- downlink telemetry/ 0      
4- RC channels 1_8_1/  4        
5 -uplink telemetry  /    5            
6- downlink telemetry/  0      
7- RC channels 1_8_2/         
8- RC channels 1_8_1/          
9- downlink telemetry/            
...
14- RC channels 1_8/              
15- downlink telemetry/
16- RC channels 1_8/5             
17  uplink telemetry /0              
15- downlink telemetry/
