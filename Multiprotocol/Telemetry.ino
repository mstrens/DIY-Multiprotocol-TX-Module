...
void frskySendStuffed()  // send a Frsky frame to handset with first a start byte, all stuffed bytes and a stop
{
...
*/
    ...
    {   // here we process a frame downlink tlm frame being received 
        ...
        {//Sequence is ok
            ...
            uint8_t count ;
            count = ( buffer[0] >> 4) & 0x0F; // number of sport bytes is the 4 MSB bits
            ...
            {
                p->count = 0;          // save the number of bytes to be handle later to be transmitted on Sport bus.
                buffer[0] &= 0x0F;     // Discard packet setting number of byte = 0
            }
            p->valid = true;
        }
    }  // end protocol Milo
...
void sportSendFrame()  // create a frame from pktx1[] and sent it using multiprotocol or sport format 
{
    ...
    sportSend(frame); // send frame[] to the handset in the Multiprotocol format or the sport original format
}   
...
void TelemetryUpdate()
{
    ...
    */
        ...
        {
            multi_send_status();  // send a Multiprotocol Status back to handset once per 500msec
            ...
            {
                mult_send_inputsync(); // send a Multiprotocol Synchro back to handset once per 100msec
                lastInputSync = now;
                return;
            }
        #endif
        ...
        {   // FrSkyX
            ...
            { //Empty buffer
                struct t_FrSkyX_RX_Frame *p ;
                uint8_t count ;
                p = &FrSkyX_RX_Frames[FrSkyX_RX_NextFrame] ; // this array contains the data received by RF
                if ( p->valid )  // valid says if the data in the array are OK
                {
                    count = p->count ;  
                    for (uint8_t i=0; i < count ; i++)
                        proces_sport_data(p->payload[i]) ;
                    p->valid = false ;  // flag the occurence in array as processed
                    if(protocol != PROTO_MILO)//use only one sequence
                        FrSkyX_RX_NextFrame = ( FrSkyX_RX_NextFrame + 1 ) & 3 ; // prepare processing next occurence
                }
                else
                    break ;
            }
...
/**************************/
    ...
    #endif // defined AVR_COMMON || defined STM32_BOARD
